...
impl<'a, H: BehaviorHost> BehaviorContext<'a, H> {
    ...
    pub(crate) fn bind_host(&self, transaction: H::Transaction) -> UniverseTransaction {
    ...
    pub(crate) fn replace_self<B: Behavior<H> + 'static>(
        &self,
        new_behavior: B,
    ) -> UniverseTransaction {
...
impl<H: BehaviorHost> BehaviorSet<H> {
    ...
    pub(crate) fn query<T: Behavior<H>>(
        &self,
    ) -> impl Iterator<Item = QueryItem<'_, H, T>> + '_ {
        self.query_any(Some(TypeId::of::<T>()))
            .map(|QueryItem { attachment, behavior }| QueryItem {
                attachment,
    ...
    pub(crate) fn query_any<'a>(
        ...
        self.items
            .iter()
            .map(move |
                entry: &'a BehaviorSetEntry<H>,
            | -> QueryItem<'a, H, dyn Behavior<H> + 'static> {
                QueryItem {
            ...
            .filter(move |qi| {
                type_filter.map_or(true, |t| (*qi.behavior).type_id() == t)
            })
    }
    pub(crate) fn step(
        &self,
...
impl<'a, H: BehaviorHost, B: Behavior<H> + ?Sized> Debug for QueryItem<'a, H, B> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let QueryItem { attachment, behavior } = self;
        behavior.fmt(f)?;
        write!(f, " @ {attachment:?}")?;
        Ok(())
    }
...
mod testing {
    ...
    impl<H: BehaviorHost, D: Debug + Send + Sync + 'static> Behavior<H>
    for NoopBehavior<D> {
...
mod tests {
    ...
    impl Behavior<Character> for SelfModifyingBehavior {
        fn step(
            ...
            context
                .replace_self(SelfModifyingBehavior {
                    foo: self.foo + 1,
                })
                .merge(
                    context
                        .bind_host(
                            CharacterTransaction::body(BodyTransaction {
    ...
    fn query() {
        ...
        BehaviorSetTransaction::insert((), arc_qu.clone())
            ...
            .unwrap();
        assert_eq!(
            set.query::< Q < Expected >> ().map(| qi | qi.behavior).collect::< Vec < _ >>
            (), vec![& Q(Expected)],
        );
        assert_eq!(
            set.query_any(None).map(| qi | qi.behavior as * const dyn Behavior <
            Character >).collect::< Vec < _ >> (), vec![Arc::as_ptr(& arc_qe) as * const
            dyn Behavior < Character >, Arc::as_ptr(& arc_qu) as * const dyn Behavior <
            Character >],
        )
    }
    #[test]
    fn txn_attachments_insert() {
        let attachment = SpaceBehaviorAttachment::new(
            GridAab::from_lower_size([0, 0, 0], [1, 1, 1]),
        );
        let transaction = BehaviorSetTransaction::<
            Space,
        >::insert(attachment, Arc::new(NoopBehavior(1)));
        assert_eq!(
            transaction.attachments_affected().collect::< Vec < _ >> (), vec![&
            attachment]
        );
    }
    #[test]
    fn txn_attachments_replace() {
        let attachment1 = SpaceBehaviorAttachment::new(
            GridAab::from_lower_size([0, 0, 0], [1, 1, 1]),
        );
        let attachment2 = SpaceBehaviorAttachment::new(
            GridAab::from_lower_size([10, 0, 0], [1, 1, 1]),
        );
        let transaction = BehaviorSetTransaction::<
            Space,
        >::replace(
    ...
    fn txn_check_non_matching_old() {
        ...
        >::replace(
            ...
            Replace {
                old: BehaviorSetEntry {
                    attachment,
                    behavior: Arc::new(NoopBehavior(2)),
                },
                new: BehaviorSetEntry {
                    attachment,
        ...
        assert_eq!(
            transaction.check(& set).unwrap_err(), transaction::PreconditionFailed {
            location : "BehaviorSet", problem :
            "existing behavior value is not as expected" }
        );
        let transaction = BehaviorSetTransaction::<
            Space,
        >::replace(
            ...
            Replace {
                old: BehaviorSetEntry {
                    attachment: SpaceBehaviorAttachment::new(
                        GridAab::from_lower_size([100, 0, 0], [1, 1, 1]),
                    ),
                    behavior: Arc::new(NoopBehavior(1)),
                },
                new: BehaviorSetEntry {
        ...
        assert_eq!(
            transaction.check(& set).unwrap_err(), transaction::PreconditionFailed {
            location : "BehaviorSet", problem :
            "existing behavior attachment is not as expected" }
        );
    }
    #[test]
    fn txn_systematic() {
        ...
        transaction::TransactionTester::new()
            ...
            .transaction(
                BehaviorSetTransaction::insert((), b1),
                |_, after| {
                    after
            ...
            .transaction(
                BehaviorSetTransaction::insert((), b2),
                |_, after| {
                    after
                        .query::<SelfModifyingBehavior>()
                        .map(|item| item.behavior)
